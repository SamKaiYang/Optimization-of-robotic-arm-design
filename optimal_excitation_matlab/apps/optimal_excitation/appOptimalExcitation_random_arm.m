...
addpath(genpath('apps/optimal_excitation/functions_random_arm/'));
...
% while(1)
%     chatterpub.send(chattermsg);
%     pause(1)
% end
% strMsg = rosmessage("/communication_matlab","communication_matlab","struct")
% laser = rossubscriber("/communication_matlab","communication_matlab");
% blankMsg = rosmessage("std_msgs/String","DataFormat","struct")
% pause(2)
laser = rossubscriber("/communicate_matlab",@rosmsgCallback);
pause(2)

laser_data = receive(laser)

while laser_data.Data == "1"
    %% Options
    ...
    if plot_and_visualize
        ...
        % robot visualization
        appVisualize_random_arm(q_optimal);
    end
