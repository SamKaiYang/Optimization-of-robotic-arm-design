...
#instantaneous joint positions for respective angles.
    ...
    plt.plot(xytheta_arr[:,0],xytheta_arr[:,1],'.','MarkerEdgeColor','k')
    # plt.show()
...
X_base = 0
Y_base = 0
# TODO:make animated plots with Matplotlib and Python
for i, j in maxmin:
    # base = plt.plot(X_base,Y_base,'o','MarkerSize',5,'MarkerEdgeColor','k','MarkerFaceColor','k')
    ##setting figure initial properties
    # plt.axis('padded')
    # pbaspect(np.array([1,1,1]))
    # hold('on')
    ...
    # print(A1_x_tip[0])
    # print(np.array([X_base,A1_x_tip[0]]),np.array([Y_base,A1_y_tip[0]]))
    L1 = plt.plot(np.array([X_base,A1_x_tip[0]]),np.array([Y_base,A1_y_tip[0]]),'r','LineWidth',2)
    ...
    L2 = plt.plot(np.array([A1_x_tip,A2_x_tip]),np.array([A1_y_tip,A2_y_tip]),'g','LineWidth',2)
    ...
    L3 = plt.plot(np.array([A2_x_tip,A3_x_tip]),np.array([A2_y_tip,A3_y_tip]),'b','LineWidth',2)
    #Marking the End Effector separately  
    E_eff = plt.plot(A3_x_tip[0],A3_y_tip[0],'.','MarkerEdgeColor','y')
    #Plot properties
    # pause(0.1)
