...
class Dynamics_space():
    def __init__(self):
        ...
        # del self.tm.links[0]
        # del self.tm.links[1]
        self.tm.gravload(self.tm.qn)
        self.tm.inertia(self.tm.qn)
        ...
        self.tau_j = self.tm.rne(self.tm.qn, self.vel, self.acc)
        ...
        for i in range(N):
            for j in range(N):
                M = self.tm.inertia(np.r_[0, Q2[i,j], Q3[i,j], 0, 0, 0])
                M11[i,j] = M[0,0]
                M12[i,j] = M[0,1]
    ...
    def planned_path_callback(self, data):
        ...
        self.tm_trajectory = data.trajectory
        self.tmstate = data.trajectory_start

        points_num = len(self.tm_trajectory[0].joint_trajectory.points)
        positions = []
        velocities = []
        accelerations = []
        time_from_start = []
        ...
        for i in range(points_num):
            positions.append(self.tm_trajectory[0].joint_trajectory.points[i].positions)
            velocities.append(self.tm_trajectory[0].joint_trajectory.points[i].velocities)
            accelerations.append(self.tm_trajectory[0].joint_trajectory.points[i].accelerations)
            time_from_start.append(self.tm_trajectory[0].joint_trajectory.points[i].time_from_start)
            time_from_start_secs.append(self.tm_trajectory[0].joint_trajectory.points[i].time_from_start.secs)
            time_from_start_nsecs.append(self.tm_trajectory[0].joint_trajectory.points[i].time_from_start.nsecs)
    ...
    def payload_set(self):
        self.tm.payload(20, [0, 0, 0]) # set payload 
    ...
    def sol_output_axis(self):
        ...
        for i in range(6):
            ...
            for i in range(len(torque_where[0])):
                ...
                # TODO: 求解逆運動學 各軸角度self.tm.ikine_a()
                self.T.t[0] = self.T_x[0,i]
                self.T.t[1] = self.T_y[0,i]
                self.T.t[2] = self.T_z[0,i]
                sol = self.tm.ikine_LM(self.T)  # original sol = self.tm.ikine_a(self.T, "lun")
                print("sol:",sol)
                # self.tm.plot(sol.q, dt=0.1)
                # self.tm.plot(sol.q)
            ...
            file_name = self.xlsx_outpath+'/tm_dynamics_space_calc_axis'+str(axis+1)+'_positive'+'.xlsx'
            ...
            file_name = self.xlsx_outpath+'/tm_dynamics_space_calc_axis'+str(axis+1)+'_negative'+'.xlsx'
            excel_file.save(file_name)
    ...
    def dynamics_calc(self):
        ...
        plt.savefig(path.join(self.pic_outpath,"tm_dataname_dynamics_calc.png"))
        plt.close()
        # excel output
        # 建立excel空白活頁簿
        excel_file = Workbook()
        ...
        file_name = self.xlsx_outpath+'/tm_dynamics_calc'+'.xlsx'
    ...
    def trajectory_dynamics_calc(self, num, pos, vel, acc, time):
        ...
        self.tau_j_array = []
        for i in range(num):
            self.tau_j = self.tm.rne(pos[i], vel[i], acc[i])
    ...
    def arm_plot(self):
        ...
        self.qn = qn
        # fig = plt.figure()
        # self.tm.plot(self.qn,dt=0, block = False, loop=False)
        print("arm_plot:", self.qn)
        self.tm.plot(self.qn, backend='pyplot', block=False, vellipse=False, fellipse=False)
        # self.tm.plot(self.qt.q, backend=self.args.backend, block=False, movie="trajectory_generation.gif", vellipse=False, fellipse=False)
        plt.show()
    ...
    def dynamics_torque_limit(self):
                    ...
                    for l in range(len(q_list)):
                        ...
                        for m in range(len(q_list)):
                            ...
                            for n in range(len(q_list)):
                                ...
                                # T = self.tm.fkine([q1*du, q2*du, q3*du, q4*du, q5*du, q6*du])
                                load = np.array([self.tm.rne([q1*du, q2*du, q3*du, q4*du, q5*du, q6*du], self.vel, self.acc)])
        ...
        file_name = self.xlsx_outpath+'/tm_dynamics_limit_torque_calc'+'.xlsx'
        excel_file.save(file_name)
