        ...
        for q3 in range(q3_s, q3_end, step):
            for q4 in range(q4_s, q4_end, step):
                for q5 in range(q5_s, q5_end, step):
                    ...
                    load = p560.gravload([q1*du, q2*du, q3*du, q4*du, q5*du, 0*du])
                    # print(load)
                    torque = np.append(torque,np.array(load),axis=0)
                    # print(torque)
                    T_x[0,i] = T.t[0]
                    T_y[0,i] = T.t[1]
                    T_z[0,i] = T.t[2]
                    # ax.scatter(T_x[0,i], T_y[0,i], T_z[0,i], c='r', marker='o')
                    i=i+1
...
max_value = np.max(torque[:,0])
print('Maximum value of the array is',max_value)

# idx, max_value= max(torque, key=lambda item: item[:,0])
# print('Maximum value:', max_value, "At index:",idx)
# idx, max_value= max(torque, key=lambda item: item[:,1])
# print('Maximum value:', max_value, "At index:",idx)
# idx, max_value= max(torque, key=lambda item: item[:,2])
# print('Maximum value:', max_value, "At index:",idx)
# idx, max_value= max(torque, key=lambda item: item[:,3])
# print('Maximum value:', max_value, "At index:",idx)
# idx, max_value= max(torque, key=lambda item: item[:,4])
# print('Maximum value:', max_value, "At index:",idx)
# idx, max_value= max(torque, key=lambda item: item[:,5])
# print('Maximum value:', max_value, "At index:",idx)
# print("軸1 torque最大值:",max(torque[:,0]))
# print("軸2 torque最大值:",max(torque[:,1]))
# print("軸3 torque最大值:",max(torque[:,2]))
# print("軸4 torque最大值:",max(torque[:,3]))
# print("軸5 torque最大值:",max(torque[:,4]))
# print("軸6 torque最大值:",max(torque[:,5]))
